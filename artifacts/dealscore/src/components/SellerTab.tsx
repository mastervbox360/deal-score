...
function SbarHdr({ icon, title, subtitle, badge, badgeColor }: {
  ...
  const badgeBg  = { amber: '#fef3c7', green: TEAL_LIGHT, navy: NAVY_LIGHT }[badgeColor ?? 'navy'] ?? NAVY_LIGHT
  const badgeClr = { amber: '#92400e', green: '#065f46',  navy: NAVY       }[badgeColor ?? 'navy'] ?? NAVY
  return (
    <div style={{
            ...
            <div style={{ padding: '14px 16px' }}>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '14px' }}>
                {(['call', 'email', 'visit', 'note'] as ActType[]).map(t => (
                  <button
                    key={t}
                    onClick={() => { setSoActType(t); openSo('activity') }}
                    style={{
                      fontSize: '11px', fontWeight: 500, padding: '5px 12px',
                      borderRadius: '20px',
                      border: t === 'call' ? 'none' : `.5px solid ${DS_BORDER}`,
                      background: t === 'call' ? NAVY : '#fff',
                      color: t === 'call' ? '#fff' : TEXT_2,
                      cursor: 'pointer', fontFamily: 'inherit',
                      display: 'inline-flex', alignItems: 'center', gap: '4px',
                    }}
                  >
            ...
            <div style={{ padding: '14px 16px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                {[
                  ...
                  { lbl: 'Tenure',            val: chainTenure,     color: undefined },
                  { lbl: 'Solicitor',         val: chainSolicitor,  color: undefined },
          ...
          <div style={{
            ...
            <div style={{ background: NAVY, padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <div style={{ fontSize: '15px', fontWeight: 600, color: '#fff' }}>
                {soKind === 'activity'         ? 'Log activity'
                  : soKind === 'offer'          ? 'Log a new offer'
                  : soKind === 'edit-vendor'    ? 'Edit vendor details'
                  : soKind === 'edit-motivation'? 'Motivation & context'
                  : 'Edit chain & property'}
              </div>
              ...
              {soKind === 'edit-vendor' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {[
                    ...
                    { label: 'Email',  value: soVEmail,  onChange: setSoVEmail, type: 'email' as const },
                  ].map(f => (
                    <div key={f.label}>
