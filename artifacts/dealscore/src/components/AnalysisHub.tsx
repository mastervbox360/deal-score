...
const scOpts = (ss: string[]) => ss.map(s => ({ value: s, label: s }))
    ...
    <>
      ...
      <div style={{ marginTop: 14 }}>
        <ISelect label="Property type" value={data.propertyType ?? ''} onChange={v => set('propertyType', v)} required
          options={scOpts(['House', 'Flat', 'Bungalow', 'HMO', 'Commercial / Mixed use'])}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}>
        ...
        <ISelect label="EPC rating" value={data.epc ?? ''} onChange={v => set('epc', v)}
          options={scOpts(['A','B','C','D','E','F','G','Not yet assessed'])}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}>
        <ISelect label="Flood risk" value={data.floodRisk ?? ''} onChange={v => set('floodRisk', v)}
          options={scOpts(['Very low','Low','Medium','High','Very high','Unknown'])}
        ...
        <ISelect label="Tenure" value={data.tenure ?? 'freehold'} onChange={v => set('tenure', v)}
          options={scOpts(['Freehold','Leasehold'])}
      ...
      {financeType !== 'cash' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 14 }}>
          ...
          <ISelect label="Mortgage type" value={data.mortgageType ?? 'Interest only'} onChange={v => set('mortgageType', v)}
            options={scOpts(['Interest only','Repayment'])}
      ...
      <div style={{ marginTop: financeType !== 'cash' ? 14 : 0, paddingTop: financeType !== 'cash' ? 14 : 0, borderTop: financeType !== 'cash' ? '1px solid var(--ds-border)' : 'none' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <ISelect label="Refurb financing" value={data.refurbFinancing ?? 'Cash'} onChange={v => set('refurbFinancing', v)}
            options={scOpts(['Cash','Separate bridging facility'])}
      ...
      <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--ds-border)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <ISelect label="Ownership structure" value={data.ownershipStructure ?? 'Personal name'}
            ...
            options={scOpts(['Personal name','Ltd company','Joint (personal)','Joint (Ltd)'])}
  ...
  return (
    <div className="ds-content">
      ...
      <div style={{ position: 'sticky', top: 'calc(var(--hdr-h, 56px) + var(--istrip-h, 48px) + var(--livebar-h, 44px) + var(--tabs-h, 42px))', zIndex: 100, backgroundColor: 'var(--bg-body)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', marginBottom: 8 }}>
        ...
        {activeView === 'inputs' && (
          isEditing ? (
            <span style={{ fontSize: 11, fontWeight: 600, color: '#065f46', background: '#d1fae5', padding: '3px 10px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4, marginRight: 24 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-2)', display: 'flex', alignItems: 'center', gap: 4, marginRight: 4, paddingRight: 20 }}>
              <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
