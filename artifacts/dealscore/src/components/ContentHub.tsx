  ...
  return (
    <div className="ds-content">
      ...
      <div className="ctype-row">
        {CONTENT_TYPES.map(ct => {
          ...
          return (
            <div
              key={ct.key}
              className={`ctype-card${isActive ? ' on' : ''}${ct.disabled ? ' soon' : ''}`}
              onClick={() => {
      ...
      <div className="workspace">
        ...
        {/* Right: sticky live-preview shell */}
        <div className="wp-right">
          <div className="pv-shell">

            {/* Preview top bar */}
            <div className="pv-top">
              <div className="pv-top-label">
                <i className={`ti ${CTYPE_ICONS[contentType] ?? 'ti-file'}`} />
                {contentType === 'advert'   ? 'Advert preview'
                 : contentType === 'onepager' ? 'One-pager preview'
                 : contentType === 'privacy'  ? 'Privacy pack preview'
                 : 'Investor pack preview'}
              </div>
              <div className="pv-controls">
                {(contentType === 'privacy' || contentType === 'full') && (
                  <div className="page-nav">
                    <button className="page-nav-btn" onClick={() => goPage(-1)} disabled={currentPage <= 1}>
                      <i className="ti ti-chevron-left" style={{ fontSize: '10px' }} />
                    </button>
                    <span>{currentPage} / {totalPageCount}</span>
                    <button className="page-nav-btn" onClick={() => goPage(1)} disabled={currentPage >= totalPageCount}>
                      <i className="ti ti-chevron-right" style={{ fontSize: '10px' }} />
                    </button>
                  </div>
                )}
                <button className="pv-btn">
                  <i className="ti ti-arrows-maximize" style={{ fontSize: '10px' }} /> Full screen
                </button>
              </div>
            </div>
            ...
            {/* Page label for packs */}
            {(contentType === 'privacy' || contentType === 'full') && (
              <div style={{ textAlign: 'center', fontSize: '10px', color: DS_TEXT2, padding: '4px 0 2px' }}>
                {pages[currentPage - 1]} · Page {currentPage} of {totalPageCount}
              </div>
            )}
            ...
            <div className="pv-actions">
              {contentType === 'advert' && (
                <>
                  <button className="log-btn primary" style={{ flex: 1, justifyContent: 'center' }}>
                    <i className="ti ti-download" style={{ fontSize: '11px' }} /> Download
                  </button>
                  <button className="log-btn" style={{ flex: 1, justifyContent: 'center' }}>
                    <i className="ti ti-copy" style={{ fontSize: '11px' }} /> Copy text
                  </button>
                  <button className="log-btn">
                    <i className="ti ti-external-link" style={{ fontSize: '11px' }} /> Advert Builder
                  </button>
                </>
              )}
              {contentType === 'onepager' && (
                <>
                  <button className="log-btn primary" style={{ flex: 1, justifyContent: 'center' }}>
                    <i className="ti ti-download" style={{ fontSize: '11px' }} /> Export PDF
                  </button>
                  <button className="log-btn" style={{ flex: 1, justifyContent: 'center' }}>
                    <i className="ti ti-link" style={{ fontSize: '11px' }} /> Share link
                  </button>
                </>
              )}
              {contentType === 'privacy' && (
              ...
              {contentType === 'full' && (
                <>
                  ...
                  >
                    {({ loading: pdfLoading }: { loading: boolean }) => (
                      <button className="log-btn primary" style={{ width: '100%', justifyContent: 'center' }}>
                        <i className="ti ti-download" style={{ fontSize: '11px' }} />
                        {pdfLoading ? ' Preparing…' : ' Export & share'}
                      </button>
                    )}
